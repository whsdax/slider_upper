...
#  ------------------------------------------------------------------
#  【项目说明 - 中文】
#  本文件为 PEAK-System 官方 PCAN-Basic API 的 Python 绑定。
#  通过 ctypes 加载 PCANBasic.dll，供项目内 can.py 调用。
#  运行环境需安装 PEAK 驱动；Windows 下 DLL 名称为 PCANBasic。
#  本项目主要使用：InitializeFD、ReadFD、WriteFD、Uninitialize、GetErrorText
#  ------------------------------------------------------------------
...
# ///////////////////////////////////////////////////////////
# 【模块一】类型定义 - ctypes 与 C API 对应的 Python 类型别名
# ///////////////////////////////////////////////////////////
...
# ///////////////////////////////////////////////////////////
# 【模块二】常量定义 - 通道句柄、错误码、设备类型、参数、报文类型、波特率等
# ///////////////////////////////////////////////////////////
...
PCAN_USBBUS1 = TPCANHandle(0x51)  # PCAN-USB interface, channel 1  【本项目 can.py 默认使用此通道】
...
#
PCAN_ERROR_OK = TPCANStatus(0x00000)  # No error  【操作成功】
...
PCAN_ERROR_QRCVEMPTY = TPCANStatus(0x00020)  # Receive queue is empty  【接收队列为空，can.py recv 轮询时常见】
...
PCAN_MESSAGE_FD = TPCANMessageType(0x04)  # FD frame  【CAN-FD 帧标志，推杆通信必需】
PCAN_MESSAGE_BRS = TPCANMessageType(
    0x08
)  # FD bit rate switch  【数据段比特率切换，与 FD 组合使用】
...
# ///////////////////////////////////////////////////////////
# 【模块三】C 结构体 - 与 DLL 内存布局一致的报文/通道描述结构
# ///////////////////////////////////////////////////////////
...
class TPCANMsgFD(Structure):
    """
    Represents a PCAN message
    【CAN-FD 报文结构体，本项目 can.py Message 类底层使用此结构】
    """
...
# ///////////////////////////////////////////////////////////
# 【模块四】PCANBasic 类 - 封装 DLL 的初始化、收发、参数读写等 API
# ///////////////////////////////////////////////////////////
...
class PCANBasic:
    """
    PCAN-Basic API class implementation
    【通过 ctypes 调用 PCANBasic.dll，can.py 的 Bus 类依赖本类】
    """

    def __init__(self):
        # Loads the PCANBasic API
        # 【中文】按操作系统加载对应动态库：Windows=PCANBasic.dll
        #
    ...
    def InitializeFD(self, Channel, BitrateFD):
        ...
        【中文】初始化 CAN-FD 通道；can.Bus.__init__ 调用。成功返回 PCAN_ERROR_OK。
        """
        try:
    ...
    def Uninitialize(self, Channel):
        ...
        【中文】释放通道；can.Bus.shutdown 调用。注意：本项目 GUI 中需谨慎多次调用。
        """
        try:
    ...
    def ReadFD(self, Channel):
        ...
        【中文】从接收队列读一条 FD 报文；队列为空返回 PCAN_ERROR_QRCVEMPTY。can.Bus.recv 轮询调用。
        """
        try:
            msg = TPCANMsgFD()
    ...
    def WriteFD(self, Channel, MessageBuffer):
        ...
        【中文】发送一条 CAN-FD 报文；can.Bus.send 调用。MessageBuffer 为 TPCANMsgFD 结构体。
        """
        try:
    ...
    def GetErrorText(self, Error, Language=0):
        ...
        【中文】将错误码翻译为文本；can.Bus.GetFormattedError 在初始化失败时使用。Language=0x09 为英语。
        """
        try:
            mybuffer = create_string_buffer(256)
